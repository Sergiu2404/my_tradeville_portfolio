...
DIVIDENDS_COLUMNS = ["Date", "Dividends", "Symbol"]
...
PORTFOLIO_SNAPSHOTS_COLUMNS_MAP = {
    ...
    "AvgPrice": "NUMERIC(10, 6)",
    "MarketPrice": "NUMERIC(10, 4)",
    "PType": "VARCHAR(3)",
    "Ccy": "VARCHAR(5)",
...
# PORTFOLIO_SNAPSHOTS_COLUMNS = list(new_portfolio_snapshot_df.columns)

# new_portfolio_symbols_daily_values_df = asyncio.run(market_data_ingestor.get_portfolio_daily_data("2023-01-01", TODAY))
# print(f"size of api data: {new_portfolio_symbols_daily_values_df.size}")
...
#
# all_table_df = db_context.execute_select_query(f'SELECT * FROM {PORTFOLIO_SYMBOLS_DAILY_VALUES_TABLE}')
# print(f"size of table supabase data: {all_table_df.size}")
